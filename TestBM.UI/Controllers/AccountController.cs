    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Manage(ManageModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (mongo.ModifyBudget(model.MontlyBudget, userName)) return RedirectToAction("Index", "Home");
            }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            MongoDb mongo = new MongoDb();
            var result = mongo.Login(model.Email, model.Password, model.RememberMe, shouldLockout: false);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            {
                MongoDb mongo = new MongoDb();
                SignInStatus result = mongo.Register(model.Email, model.Password);
                if (result.Equals(SignInStatus.Success))
                {
                    var loginResult = mongo.Login(model.Email, model.Password, false, false);

                    if (loginResult.Equals(SignInStatus.Success))
                    {
                        FormsAuthentication.SetAuthCookie(model.Email, true);
